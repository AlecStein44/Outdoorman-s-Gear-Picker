...
class CampingProductSearch extends React.Component {
    ...
    handleClick(e) {
        if(sessionStorage.getItem(this.state.params.toLowerCase()) === null){
            ...
            window.location.assign('https://outdoorgearpicker.now.sh/newcampinglist')
        } else {
            let multiselected = {
    ...
    render() {
        return(
            ...
            <button className="list-back" onClick={() => { window.location.assign('https://outdoorgearpicker.now.sh/newcampinglist') }}>Back</button>
            <a onClick={this.handlePrev.bind(this) } value={this.state.prevUri} className="prevnext">Previous</a>
            <a onClick={this.handleNext.bind(this) } value={this.state.nextUri} className="prevnext">Next</a>
            </main>
        )
