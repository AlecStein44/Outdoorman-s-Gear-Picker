...
class FishingProductSearch extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            results: [],
    ...
    componentDidMount() {
        fetch(`./fishingJson/${this.state.params}.json`).then(response => {
    ...
    handleClick(e) {
        ...
        window.location.assign('https://outdoorgearpicker.now.sh/newfishinglist')
    }
    ...
    render() {
        return(
            <main>
                ...
                <button className="list-back" onClick={() => { window.location.assign('https://outdoorgearpicker.now.sh/newfishinglist') }}>Back</button>
            </main>
        )
    }
}

export default FishingProductSearch
